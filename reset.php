...
if($_SERVER['REQUEST_METHOD'] ==='POST'){
    ...
    $sql = "SELECT EmailId FROM tbljobseekers where reset_pass='".$post_token."'";
    if($query = $con->prepare($sql)){
        if($query->execute()){
            ...
            if($query->num_rows === 1){
                ...
                $sql = "UPDATE tbljobseekers SET Password='".$password_hash."' WHERE reset_pass='".$post_token."'";
                if($query = $con->prepare($sql)){
                    if($query->execute()){

                        // delete the token to avoid reuse
                        $sql = 'UPDATE tbljobseekers SET reset_pass= "" WHERE EmailId="'.$email.'"';
                        if($query = $con->prepare($sql)){
                            if($query->execute()){
